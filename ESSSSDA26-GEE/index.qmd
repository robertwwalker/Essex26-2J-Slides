...
3. **Incidental Parameter Problem**: In non-linear models with fixed $T$ as $N 	o \infty$, estimating $N$ parameters $ lpha_i$ yields inconsistent estimates for structural parameters $ eta$.
...
- Models response **conditional on individual random effect**:
  $$E[Y_{it} \mid X_{it}, u_i] = g^{-1}(X_{it} eta_{	ext{cond}} + u_i)$$
...
- Models the **population average**:
  $$E[Y_{it} \mid X_{it}] = \mu_{it} = g^{-1}(X_{it} eta_{	ext{marg}})$$
...
::: {.callout-tip}
For linear models, $ eta_{	ext{cond}} =  eta_{	ext{marg}}$. For non-linear models (e.g., Logistic), $| eta_{	ext{cond}}| > | eta_{	ext{marg}}|$ due to attenuation from integrating over the random effect distribution.
:::
...
$$g(\mu_{it}) = X_{it} eta \implies \mu_{it} = g^{-1}(X_{it} eta)$$
...
## Working Correlation Matrices $R( lpha)$

The covariance matrix for subject $i$ is modeled as:
$$V_i = \phi \, A_i^{1/2} \, R_i( lpha) \, A_i^{1/2}$$

where $A_i$ is a diagonal matrix of variance functions $v(\mu_{it})$ and $R_i( lpha)$ is the **working correlation matrix**.

### Common Choices for $R( lpha)$:
1. **Independence**: $R_{ts} = 0$ ($t 
eq s$) $\implies$ Assumes no intra-subject correlation.
2. **Exchangeable (Compound Symmetry)**: $R_{ts} =  lpha$ $\implies$ Equal correlation across all waves.
3. **Autoregressive (AR-1)**: $R_{ts} =  lpha^{|t-s|}$ $\implies$ Correlation decays over time.
4. **Unstructured**: $R_{ts} =  lpha_{ts}$ $\implies$ Fully general $T 	imes T$ correlation matrix.
...
The parameter vector $ eta$ is solved via:
$$\sum_{i=1}^N D_i^T V_i^{-1} \left( Y_i - \mu_i 
ight) = 0$$

where $D_i = rac{\partial \mu_i}{\partial  eta}$ is the matrix of partial derivatives.

### The "Huber-White" Sandwich Variance
Even if $R( lpha)$ is incorrectly specified, the asymptotic variance of $\hat{ eta}$ is consistently estimated by the **sandwich estimator**:
$$	ext{Var}(\hat{ eta}) = M_0^{-1} M_1 M_0^{-1}$$
...
| :--- | :--- | :--- | :--- |
| **Binary** | Logit: $\ln\left(rac{\mu}{1-\mu}
ight)$ | $\mu(1-\mu)$ | Voter turnout, reform adoption, employment status |
| **Count** | Log: $\ln(\mu)$ | $\mu$ (Poisson) or $\mu + k\mu^2$ (NegBin) | Protest frequency, legislative bills passed, conflict fatalities |
| **Ordinal** | Cumulative Logit: $\ln\left(rac{P(Y \le k)}{P(Y > k)}
ight)$ | $\mu(1-\mu)$ | Policy support levels, institutional trust (1-5 Likert) |
...
| :--- | :--- | :--- | :--- |
| **Target Parameter** | Conditional ($ eta_i$) | Conditional ($ eta_{	ext{cond}}$) | **Marginal / Population Avg ($ eta_{	ext{marg}}$)** |
...
Include subject-level means of time-varying predictors ($ ar{X}_i$):
$$g(\mu_{it}) = X_{it} eta_{	ext{within}} +  ar{X}_i  eta_{	ext{between}} + Z_i \gamma$$

::: {.callout-tip}
### Benefits of Hybrid GEE:
1. **Identifies Within-Effects**: $\hat{ eta}_{	ext{within}}$ yields estimates equivalent to Fixed Effects.
