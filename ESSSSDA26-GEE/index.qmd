...
3. **Incidental Parameter Problem**: In non-linear models with fixed $T$ as $N 	o \infty$, estimating $N$ parameters $\alpha_i$ yields inconsistent estimates for structural parameters $\eta$.
...
1. **Independence**: $R_{ts} = 0$ ($t \eq s$) $\implies$ Assumes no intra-subject correlation.
...
where $D_i = \frac{\partial \mu_i}{\partial  \eta}$ is the matrix of partial derivatives.

### The "Huber-White" Sandwich Variance
Even if $R(\alpha)$ is incorrectly specified, the asymptotic variance of $\hat{\eta}$ is consistently estimated by the **sandwich estimator**:
...
| :--- | :--- | :--- | :--- |
| **Binary** | Logit: $\ln\left(\frac{\mu}{1-\mu}
...
| **Ordinal** | Cumulative Logit: $\ln\left(\frac{P(Y \le k)}{P(Y > k)}
